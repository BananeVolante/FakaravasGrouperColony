...
std::list<std::array<Vector3f, 3>> Renderer::render(multithreadType mtType)
{
    ...
    auto &rQueue = renderQueue;

    switch (mtType)
    {
    case multithreadType::limited:
        for (size_t i = 0; i < MAX_THREAD_NUMBER; i++)
        {
            int start = (i * rQueue.size()) / MAX_THREAD_NUMBER;
            int end = ((i + 1) * rQueue.size()) / MAX_THREAD_NUMBER;
            threadList.push_back(std::async(std::launch::async, [rQueue, this, start, end]() -> triangleList
                                            { return this->render(rQueue, start, end); }));
        }

        break;
    case multithreadType::unlimited:
        while (!renderQueue.empty())
        {
            mesh = renderQueue.front();
            renderQueue.pop_front();
            // add the the rendered triangles to the list of triangles to render
            threadList.push_back(std::async(std::launch::async, [mesh, this]() -> triangleList
...
std::list<std::array<Vector3f, 3>> Renderer::render(const std::deque<const AbstractMesh *> &renderQueue, size_t start, size_t end) const
{
    const AbstractMesh *mesh;
    std::list<std::array<Vector3f, 3>> tList;
